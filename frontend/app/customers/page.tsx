...
import { Users, UserMinus, UserCheck, Shield, Sparkles, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-paytm-sm">
          <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-1">
            Total Customers
          </span>
          <div className="text-2xl font-extrabold text-slate-900">{metrics.total_customers}</div>
          <span className="text-[11px] text-slate-500">{metrics.new_customers} new this month</span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-paytm-sm">
          ...
          <span className="text-[11px] text-emerald-700">{metrics.returning_customers} loyal customers</span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-paytm-sm">
          ...
          <span className="text-[11px] text-amber-800">Target for re-engagement</span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-paytm-sm">
          ...
          <span className="text-[11px] text-red-700">60+ days without order</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-paytm-sm overflow-hidden flex flex-col justify-between">
              ...
              <tbody className="divide-y divide-slate-100">
                {atRiskQuery.data.map((cust) => (
                  <tr key={cust.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-3 text-slate-600">{formatDate(cust.last_order_date)}</td>
                    <td className="p-3 font-bold text-slate-800">{cust.total_orders}</td>
                    <td className="p-3 font-extrabold text-slate-900">{formatCurrency(cust.total_spend)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
