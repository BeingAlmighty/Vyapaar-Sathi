...
import { Package, AlertTriangle, CheckCircle2, TrendingUp, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-rose-50/50 border border-rose-200/70 rounded-2xl p-5 flex items-center space-x-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
          ...
          <div>
            ...
            <span className="text-xs text-rose-700 font-medium">Immediate reorder required</span>
          </div>
        </div>

        <div className="bg-amber-50/50 border border-amber-200/70 rounded-2xl p-5 flex items-center space-x-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
          ...
          <div>
            ...
            <span className="text-xs text-amber-800 font-medium">Overstocked ingredients</span>
          </div>
        </div>

        <div className="bg-emerald-50/50 border border-emerald-200/70 rounded-2xl p-5 flex items-center space-x-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
          ...
          <div>
            ...
            <span className="text-xs text-emerald-700 font-medium">Optimal operating levels</span>
          </div>
        </div>
      </div>

      {/* Inventory Items Table */}
      <div className="bg-white border border-slate-200/70 rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] overflow-hidden">
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-100/70 text-slate-600 font-bold border-b border-slate-200/70 uppercase text-[10px]">
                <th className="p-4">Product / Ingredient</th>
                <th className="p-4">Category</th>
                <th className="p-4">Current Stock</th>
                <th className="p-4">Min - Max Level</th>
                <th className="p-4">Status</th>
                <th className="p-4">Rec. Reorder</th>
              ...
              {data.items.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-4 font-extrabold text-slate-900">{item.current_stock}</td>
                    ...
                    <td className="p-4 font-bold text-slate-900">
                      {item.recommended_reorder_qty > 0 ? (
                        <span className="text-rose-600">+{item.recommended_reorder_qty} units</span>
                      ) : (
                    ...
                    <td className="p-4 text-right space-x-2">
                      {isLowStock ? (
                        <Link
                          ...
                          className="inline-flex items-center px-3 py-1.5 bg-rose-600 text-white font-bold rounded-xl hover:bg-rose-700 transition-colors text-xs"
                        >
                          Restock Now
                        </Link>
                      ) : isExcessStock ? (
                        <Link
                          ...
                          className="inline-flex items-center px-3 py-1.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-colors text-xs"
                        >
                          Create Combo
                        </Link>
                      ) : (
